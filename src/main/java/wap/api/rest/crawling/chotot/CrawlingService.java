...
public class CrawlingService implements ICrawlingService {
  ...
  @Autowired
  public CrawlingService(@Qualifier("chototCrawlingDao") ICrawlingDao crawlingDao) {
...
//          crawlingDao.trackingItem(crawlingTracking.getId(), item.getId());
  ...
  private void getTrackingAndItems(String pageLink, Map<String, CrawlingTracking> crawlingTrackingMap) {
    try {
      ...
      String categoryName = document.select("").get(0).select("div.Title").select("h1").get(0).text();
